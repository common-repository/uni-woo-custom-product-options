...
class Uni_Cpo_Module_Row extends Uni_Cpo_Module implements Uni_Cpo_Module_Interface {
	...
	public static function js_template() {
		...
		<script id="js-builderius-row-tmpl" type="text/template">
            ...
            {{ const { display = 'block', flex_direction, justify_content, flex_wrap, align_items, gap = {}, width_type, width, content_width } = general.main; }}
            {{ const { value: gap_value = '', unit: gap_unit = '' } = gap; }}
            ...
            <div id="{{- id_name }}" class="uni-row uni-node-{{= id }} {{- class_name }}" data-node="{{= id }}" data-type="{{= type }}">
                <style>
                    ...
                    .uni-node-{{= id }} .uni-row-content-wrap .uni-row-content {
                                            display: {{= display }};
                ...
                <div class="uni-row-content-wrap">
                    <div id="js-row-group-{{= id }}" class="uni-row-content uni-row-content-{{= content_width }} uni-row-content-{{= display }} uni-node-content"></div>
                </div>
            </div>
		</script>
		<?php
	...
	public static function template( $data, $post_data = array() ) {
        ...
        <div
            ...
            <div class="uni-row-content-wrap">
                ...
                <div class="uni-row-content uni-node-content uni-row-content-<?php echo !empty($main['display']) ? esc_attr($main['display']) : 'block'; ?>">
					<?php
	...
	public static function get_css( $data ) {
        ...
        .uni-node-<?php echo esc_attr( $id ); ?> .uni-row-content-wrap .uni-row-content {
            display: <?php echo !empty($main['display']) ? esc_attr($main['display']) : 'block'; ?>;
        <?php if (!empty($main['display']) && in_array($main['display'], ['flex', 'inline-flex'])) { ?>
            flex-direction: <?php echo !empty($main['flex_direction']) ? esc_attr($main['flex_direction']) : 'row'; ?>;
            justify-content: <?php echo !empty($main['justify_content']) ? esc_attr($main['justify_content']) : 'inherit'; ?>;
            flex-wrap: <?php echo !empty($main['flex_wrap']) ? esc_attr($main['flex_wrap']) : 'nowrap'; ?>;
            align-items: <?php echo !empty($main['align_items']) ? esc_attr($main['align_items']) : 'inherit'; ?>;
        <?php if (!empty($main['gap'])) { ?>
            gap: <?php echo !empty($main['gap']['value']) ? esc_attr($main['gap']['value']) : 0; ?><?php echo !empty($main['gap']['unit']) ? esc_attr($main['gap']['unit']) : 'px'; ?>;
        <?php } ?>
        <?php } ?>
        }
		<?php if ( $links['color'] ) { ?>
