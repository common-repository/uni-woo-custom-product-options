...
add_action(
    ...
    'uni_cpo_add_custom_settings_tab_content',
    10,
    0
);
function uni_cpo_add_custom_settings_tab_content() {
    ...
    ?>
        ...
        <div class="uni_cpo_product_onboarding">
            <h4><?php 
    esc_html_e( 'First time using Uni CPO?', 'uni-cpo' );
    ?></h4>
            <p><?php 
    esc_html_e( 'You may import one of our ready-made configurations of the commonly used product setups', 'uni-cpo' );
    ?></p>
            <div class="uni_cpo_product_onboarding__form">
                <input type="hidden" name="cpo_security" value="<?php 
    echo wp_create_nonce( 'uni_cpo_builder' );
    ?>"/>
                <select id="unicpo_onboarding_configs" name="unicpo_configs">
                    <?php 
    ?>
                        <option value="carpet_basic_free">Selling carpets (basic area calculation)</option>
                        <option value="tshirt_form">Selling t-shirt (size/color options)</option>
                        <option value="1" disabled>Selling windows (many options, complex calculation) (PRO version)</option>
                        <option value="2" disabled>Selling a smartwatch ("imagify" functionality) (PRO version)</option>
                        <option value="3" disabled>Selling a gamepad ("colorify" functionality; REQUIRED added any product image) (PRO version)</option>
                        <option value="4" disabled>Selling dumbbells (PRO version)</option>
                        <option value="5" disabled>Selling floor tiles/parquet (PRO version)</option>
                        <option value="6" disabled>Selling a notebook (color & qty of pages) (PRO version)</option>
                    <?php 
    ?>
                </select>
                <button id="unicpo_onboarding_import" type="button"><?php 
    esc_html_e( 'Import configuration', 'uni-cpo' );
    ?></button>
            </div>
            <div>
                <p>
                    <a href="https://moomoo-agency.gitbook.io/uni-cpo-4-documentation" target="_blank">
                        <?php 
    esc_html_e( 'Link to the documentation', 'uni-cpo' );
    ?>
                    </a> |
                    <a href="https://moomoo.agency/tutorials-and-useful-resources/" target="_blank">
                        <?php 
    esc_html_e( 'List of tutorials and articles', 'uni-cpo' );
    ?>
                    </a>
                </p>
            </div>
