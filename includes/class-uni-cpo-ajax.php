...
class Uni_Cpo_Ajax {
    ...
    public static function uni_cpo_save_model() {
        try {
            ...
            $data = $_POST['model'];
            $result = uni_cpo_save_modal( $data );
            if ( $result['success'] ) {
    ...
    public static function uni_cpo_price_calc() {
        try {
            ...
            $price_vars['quantity'] = ( !empty( $form_data['quantity'] ) ? ( (float) $form_data['quantity'] < 0 ? (float) $form_data['quantity'] * -1 : (float) $form_data['quantity'] ) : 1 );
            ...
            if ( 'on' === $product_data['settings_data']['cpo_enable'] && 'on' === $product_data['settings_data']['calc_enable'] ) {
                ...
                if ( !$is_calc_disabled ) {
                    ...
                    if ( function_exists( 'UniCpoCustomSku' ) ) {
                        $dynamic_sku = UniCpoCustomSku()->get_sku( $product_id, $filtered_form_data );
                        if ( $dynamic_sku ) {
                            $price_vars['dynamic_sku'] = $dynamic_sku;
                            $enable_stock = get_post_meta( $product_id, '_uni_cpo_customsku_stock_enable', true );
                            if ( $enable_stock === 'yes' ) {
                                $price_vars['stock_qty'] = UniCpoCustomSku()->get_stock_qty( $product_id, $dynamic_sku );
                                if ( $price_vars['stock_qty'] - $price_vars['quantity'] >= 0 ) {
                                    $price_vars['stock_status'] = 'instock';
                                    $price_vars['stock_status_msg'] = __( 'In stock', 'woocommerce' );
                                } else {
                                    $price_vars['stock_status'] = 'outofstock';
                                    $price_vars['stock_status_msg'] = __( 'Out of stock', 'woocommerce' );
                                    $is_calc_disabled = true;
                                }
                            }
                        }
    ...
    /**
     *   uni_cpo_onboarding_import
     */
    public static function uni_cpo_onboarding_import() {
        try {
            check_ajax_referer( 'uni_cpo_builder', 'security' );
            $product_id = absint( $_POST['pid'] );
            $file_path = UniCpo()->plugin_path() . '/includes/configs/' . $_POST['cfg_name'] . '.txt';
            if ( !isset( $product_id ) ) {
                throw new Exception(__( 'Product ID is not set', 'uni-cpo' ));
            }
            ...
            if ( $wp_filesystem->exists( $file_path ) ) {
                ...
                throw new Exception(__( 'Product configuration file does not exist', 'uni-cpo' ));
            }
            unset($content['id']);
            ...
            $builder_content = [];
            $slug_mapping = [];
            $unique_suffix = substr( uniqid(), 0, 5 );
            if ( is_array( $content['content'] ) && !empty( $content['content'] ) ) {
                array_walk( $content['content'], function ( $row, $row_key ) use(&$builder_content, &$slug_mapping, $unique_suffix) {
                    $row['pid'] = '';
                    ...
                    if ( is_array( $row['columns'] ) && !empty( $row['columns'] ) ) {
                        array_walk( $row['columns'], function ( $column, $column_key ) use(
                            ...
                            if ( is_array( $column['modules'] ) && !empty( $column['modules'] ) ) {
                                array_walk( $column['modules'], function ( $module, $module_key ) use(
                                    &$builder_content,
                                    &$slug_mapping,
                                    $row_key,
                                    $column_key,
                                    ...
                                    if ( 'option' === $module['obj_type'] ) {
                                        $module['pid'] = '';
                                        $existing_slug = $module['settings']['cpo_general']['main']['cpo_slug'];
                                        $new_slug = $existing_slug . '_' . $unique_suffix;
                                        $slug_mapping["uni_cpo_{$existing_slug}"] = "uni_cpo_{$new_slug}";
                                        $module['settings']['cpo_general']['main']['cpo_slug'] = $new_slug;
                                        $result = uni_cpo_save_modal( $module );
                                        if ( $result['success'] ) {
                                            $new_module = $result['data'];
                                            $new_module['id'] = $module['id'];
                                            $new_module['parentRowId'] = $module['parentRowId'];
                                            $new_module['parentColumnId'] = $module['parentColumnId'];
                                            $new_module['type'] = $module['type'];
                                            $new_module['obj_type'] = $module['obj_type'];
                                            $builder_content[$row_key]['columns'][$column_key]['modules'][$module_key] = $new_module;
                                        } else {
                                            if ( !empty( $result['data']['errorData']['slug'] ) ) {
                                                $new_slug = $result['data']['errorData']['slug'];
                                                $slug_mapping["uni_cpo_{$existing_slug}"] = "uni_cpo_{$new_slug}";
                                                $module['settings']['cpo_general']['main']['cpo_slug'] = $new_slug;
                                                $result2 = uni_cpo_save_modal( $module );
                                                if ( $result2['success'] ) {
                                                    $new_module = $result2['data'];
                                                    $new_module['id'] = $module['id'];
                                                    $new_module['parentRowId'] = $module['parentRowId'];
                                                    $new_module['parentColumnId'] = $module['parentColumnId'];
                                                    $new_module['type'] = $module['type'];
                                                    ...
                                                    throw new \Exception(__( 'Sorry, product configuration import was not successful', 'uni-cpo' ));
                                                }
                                            } else {
                                                throw new \Exception(__( 'Sorry, product configuration import was not successful', 'uni-cpo' ));
                                            }
            ...
            foreach ( $slug_mapping as $old_val => $new_val ) {
                $content = json_decode( str_replace( $old_val, $new_val, json_encode( $content ) ), ARRAY_A );
            }
            if ( 'smartwatch_imagify' === $_POST['cfg_name'] ) {
                $attachment_id = media_sideload_image( 'https://cpo.builderius.io/wp-content/uploads/2021/02/watch.jpg', $product_id, 'Smartwatch' );
                $content['settings_data']['imagify_base_image'] = $attachment_id;
            }
            ...
            if ( $content_saved && $settings_saved ) {
                ...
                wp_send_json_error( [
                    'error' => __( 'Something went wrong', 'uni-cpo' ),
                ] );
            }
        } catch ( Exception $e ) {
            wp_send_json_error( [
